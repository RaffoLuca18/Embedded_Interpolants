"""
turbolence_script_pca.py
=====================
Self-contained: Embedded Interpolants on 2D turbulence vorticity fields
(e.g. Johns Hopkins Turbulence Database, MHD).

Expected input:
  data/turbulence/fields.npy   shape (N, H, W),  vorticity slices

Outputs (figs/turbulence/):
...
# ════════════════════════════════════════════════════════════════════
NAME          = "2D turbulence"
DATA_PATH     = Path("data/turbulence/fields.npy")
OUT           = Path("figs/turbulence"); OUT.mkdir(parents=True, exist_ok=True)

SEED          = 42
TEST_FRACTION = 0.10

LOG_XFORM     = False         # vorticity is signed, no log
D_PCA         = 128
...
def load_fields():
    if not DATA_PATH.exists():
        print(f"\n[turbulence] data missing: {DATA_PATH}")
        print("""    To get 2D turbulence vorticity slices:
      Option A — Johns Hopkins Turbulence Database (JHTDB), public, MHD:
        pip install pyJHTDB
        # request 256x256x1 vorticity cutouts at random (t, z) slices
      Option B — Allys et al. 2019 dataset (MHD, anisotropic):
        contact authors directly.
    Save the result as data/turbulence/fields.npy with shape (N, H, W).""")
        sys.exit(1)
...
def run():
    ...
    with Heartbeat("fit"):
        model.fit(np.random.randn(N_TRAIN, d_pca).astype(np.float32),
                  Z_target_fit_s, n_iterations=N_ITER, verbose=False)

    print("PHASE 4 -- transport (fresh)")
    with Heartbeat("transport"):
        res = model.transport(
            np.random.randn(N_FRESH, d_pca).astype(np.float32))

    fresh_pca   = res['particles']
    fresh_pix_n = pca.inverse_transform(fresh_pca * sigma_z)
