...
For each digit produces:
  figs/mnist_per_digit/<d>/target.png
  figs/mnist_per_digit/<d>/iterations.png
  figs/mnist_per_digit/<d>/diagnostic.png
...
# ════════════════════════════════════════════════════════════════════
SEED          = 42
OUT_ROOT      = Path("figs/mnist_per_digit")
...
# ════════════════════════════════════════════════════════════════════
# style (shared with mnist_script.py)
# ════════════════════════════════════════════════════════════════════
mpl.rcParams.update({
    'font.family':       'serif',
...
# ════════════════════════════════════════════════════════════════════
# trace helpers (same as mnist_script.py)
# ════════════════════════════════════════════════════════════════════
...
def plot_diagnostic(out, sw_train, sw_fresh, iter_bnd, digit):
    ...
    ax.set_title(f'MNIST digit {digit}', pad=10)
    plt.tight_layout()
...
def run_digit(digit, X, y, d, rng):
    ...
    return dict(digit=digit, n_train=int(n_tr),
...
def main():
    ...
    print("PHASE 2 -- per-digit fits")
    results = [run_digit(digit, X, y, d, rng) for digit in DIGITS]

    print("PHASE 3 -- summary")
    lines  = ["MNIST per-digit", "=" * 40,
              f"{'digit':>5}  {'n_train':>7}  "
              f"{'sw1_train':>10}  {'sw1_fresh':>10}"]
    for r in results:
